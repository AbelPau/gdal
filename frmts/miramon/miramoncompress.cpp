...
MMRCompress::MMRCompress(void *pData, GUInt32 nBlockSize, EPTType eDataType)
    : m_pData(pData), m_nBlockSize(nBlockSize),
      m_nBlockCount((nBlockSize * 8) / MMRGetDataTypeBits(eDataType)),
      m_eDataType(eDataType), m_nDataTypeNumBits(MMRGetDataTypeBits(eDataType)),
...
// the data type.
GUInt32 MMRCompress::valueAsUInt32(GUInt32 iPixel)
{
    GUInt32 val = 0;
...
GUInt32 MMRCompress::findMin(GByte *pNumBits)
{
...
void MMRCompress::makeCount(GUInt32 count, GByte *pCounter,
                            GUInt32 *pnSizeCount)
{
...
void MMRCompress::encodeValue(GUInt32 val, GUInt32 repeat)
{
...
bool MMRCompress::compressBlock()
{
    ...
    if (!QueryDataTypeSupported(m_eDataType))
    {
        CPLDebug("MiraMonRaster",
                 "Cannot compress MiraMonRaster datatype 0x%x (0x%x bits). "
...
bool MMRCompress::QueryDataTypeSupported(EPTType eMMRDataType)
{
    const int nBits = MMRGetDataTypeBits(eMMRDataType);

...
MMRType::MMRType() : bInCompleteDefn(false), nBytes(0), pszTypeName(nullptr)
{
}
...
const char *MMRType::Initialize(const char *pszInput)

{
    if (*pszInput != '{')
    {
        if (*pszInput != '\0')
            CPLDebug("MMRType", "Initialize(%60.60s) - unexpected input.",
                     pszInput);
    ...
    {
        auto poNewField = std::make_unique<MMRField>();
...
bool MMRType::CompleteDefn(MMRDictionary *poDict)

{
    ...
    if (bInCompleteDefn)
    {
        CPLError(CE_Failure, CPLE_AppDefined,
                 "Recursion detected in MMRType::CompleteDefn()");
        return false;
    }
...
void MMRType::Dump(FILE *fp)

{
    CPL_IGNORE_RET_VAL(
        VSIFPrintf(fp, "MMRType %s/%d bytes\n", pszTypeName, nBytes));
...
CPLErr MMRType::SetInstValue(const char *pszFieldPath, GByte *pabyData,
...
int MMRType::GetInstCount(const char *pszFieldPath, GByte *pabyData,
...
/*      MMRField, but this method does the field name parsing.          */
/*                                                                      */
...
bool MMRType::ExtractInstValue(const char *pszFieldPath, GByte *pabyData,
                               GUInt32 nDataOffset, int nDataSize,
...
void MMRType::DumpInstValue(FILE *fpOut, GByte *pabyData, GUInt32 nDataOffset,
...
int MMRType::GetInstBytes(GByte *pabyData, int nDataSize,
                          std::set<MMRField *> &oVisitedFields) const

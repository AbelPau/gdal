...
CPLErr MMRAuxBuildOverviews(const char *pszOvrFilename, GDALDataset *poParentDS,
...

{
    ...
    if (*ppoODS == nullptr)
    {
            ...
            else
            {
                if (eDT != poBand->GetRasterDataType())
                {
                    CPLError(CE_Failure, CPLE_NotSupported,
                             "MMRAuxBuildOverviews() doesn't support a "
        ...
        // ·$·TODO cal veure si crear el REL així.
        // Create the HFA (.aux) file.  We create it with
        // COMPRESSED=YES so that no space will be allocated for the
        // base band.
        GDALDriver *poMMRDriver =
            static_cast<GDALDriver *>(GDALGetDriverByName("MiraMonRaster"));
        if (poMMRDriver == nullptr)
        {
            CPLError(CE_Failure, CPLE_AppDefined,
                     "MiraMonRaster driver is unavailable.");
            return CE_Failure;
        }
        ...
        *ppoODS =
            poMMRDriver->Create(pszOvrFilename, poParentDS->GetRasterXSize(),

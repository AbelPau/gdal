 ...
 * miramonentry.cpp
 *
 * Implementation of the MMREntry class.
 *
 */
...
MMREntry::MMREntry()
    : bDirty(false), nFilePos(0), psMMR(nullptr), poParent(nullptr),
...
/*                              MMREntry()                              */
/*                                                                      */
/*      Construct an MMREntry from the source file.                     */
/************************************************************************/

MMREntry *MMREntry::New(MMRInfo_t *psMMRIn, GUInt32 nPos, MMREntry *poParentIn,
                        MMREntry *poPrevIn)

{
    MMREntry *poEntry = new MMREntry;
    poEntry->psMMR = psMMRIn;
    ...
    if (VSIFSeekL(poEntry->psMMR->fp, poEntry->nFilePos, SEEK_SET) == -1 ||
        VSIFReadL(anEntryNums, sizeof(GInt32) * 6, 1, poEntry->psMMR->fp) < 1)
    {
        CPLError(CE_Failure, CPLE_FileIO,
                 "VSIFReadL(%p,6*4) @ %u failed in MMREntry().\n%s",
                 poEntry->psMMR->fp, poEntry->nFilePos, VSIStrerror(errno));
        delete poEntry;
        return nullptr;
    }

    for (int i = 0; i < 6; i++)
        MMRStandard(4, anEntryNums + i);
    ...
    if (VSIFReadL(poEntry->szName, 64, 1, poEntry->psMMR->fp) < 1 ||
        VSIFReadL(poEntry->szType, 32, 1, poEntry->psMMR->fp) < 1)
    {
        poEntry->szName[sizeof(poEntry->szName) - 1] = '\0';
        poEntry->szType[sizeof(poEntry->szType) - 1] = '\0';
        CPLError(CE_Failure, CPLE_FileIO, "VSIFReadL() failed in MMREntry().");
        delete poEntry;
        return nullptr;
    }
    poEntry->szName[sizeof(poEntry->szName) - 1] = '\0';
...
/*                              MMREntry()                              */
/*                                                                      */
/*      Construct an MMREntry in memory, with the intention that it     */
/*      would be written to disk later.                                 */
/************************************************************************/

MMREntry::MMREntry(MMRInfo_t *psMMRIn, const char *pszNodeName,
                   const char *pszTypeName, MMREntry *poParentIn)
    : nFilePos(0), psMMR(psMMRIn), poParent(poParentIn), poPrev(nullptr),
...
MMREntry *MMREntry::New(MMRInfo_t *psMMRIn, const char *pszNodeName,
                        const char *pszTypeName, MMREntry *poParentIn)
{
    CPLAssert(poParentIn != nullptr);
    return new MMREntry(psMMRIn, pszNodeName, pszTypeName, poParentIn);
}

/************************************************************************/
/*                      BuildEntryFromMIFObject()                       */
/*                                                                      */
/*      Create a pseudo-MMREntry wrapping a MIFObject.                  */
/************************************************************************/

MMREntry *MMREntry::BuildEntryFromMIFObject(MMREntry *poContainer,
                                            const char *pszMIFObjectPath)
{
    ...
    return new MMREntry(osDictionary, osType, nMIFObjectSize, l_pabyData);
}
...
MMREntry::MMREntry(const char *pszDictionary, const char *pszTypeName,
      ...
      nDataSize(0), bIsMIFObject(true)
{
    ...
    psMMR->poDictionary = new MMRDictionary(pszDictionary);

    // Work out the type for this MIFObject.
    memset(szType, 0, sizeof(szType));
    snprintf(szType, sizeof(szType), "%s", pszTypeName);

    poType = psMMR->poDictionary->FindType(szType);
...
CPLErr MMREntry::RemoveAndDestroy()
...
void MMREntry::SetName(const char *pszNodeName)
{
    memset(szName, 0, sizeof(szName));
...
MMREntry *MMREntry::GetChild()

{
    // Do we need to create the child node?
    if (poChild == nullptr && nChildPos != 0)
    {
        poChild = MMREntry::New(psMMR, nChildPos, this, nullptr);
        if (poChild == nullptr)
            nChildPos = 0;
    }
...

{
    ...
    if (poNext == nullptr && nNextPos != 0)
    {
        ...
        poNext = MMREntry::New(psMMR, nNextPos, poParent, this);
        if (poNext == nullptr)
            nNextPos = 0;
    }
...

{
    ...
    if (VSIFSeekL(psMMR->fp, nDataPos, SEEK_SET) < 0)
    {
        CPLError(CE_Failure, CPLE_FileIO,
                 "VSIFSeekL() failed in MMREntry::LoadData().");
        return;
    }

    if (VSIFReadL(pabyData, nDataSize, 1, psMMR->fp) < 1)
    {
        CPLError(CE_Failure, CPLE_FileIO,
                 "VSIFReadL() failed in MMREntry::LoadData().");
        return;
    }
...
MMRType *MMREntry::GetTypeObject()
...
/*                              MakeData()                              */
/*                                                                      */
/*      Create a data block on the this MMREntry in memory.  By         */
...
GByte *MMREntry::MakeData(int nSize)
...
void MMREntry::DumpFieldValues(FILE *fp, const char *pszPrefix)
...
std::vector<MMREntry *> MMREntry::FindChildren(const char *pszName,
                                               const char *pszType,
                                               int nRecLevel,
                                               int *pbErrorDetected)

{
    std::vector<MMREntry *> apoChildren;
    ...
    for (MMREntry *poEntry = GetChild(); poEntry != nullptr;
         poEntry = poEntry->GetNext())
    {
        std::vector<MMREntry *> apoEntryChildren;
...
MMREntry *MMREntry::GetNamedChild(const char *pszName)
...
bool MMREntry::GetFieldValue(const char *pszFieldPath, char chReqType,
...

{
    ...
    {
        MMREntry *poEntry = GetNamedChild(pszFieldPath);
        if (poEntry == nullptr)
            return false;
...
int MMREntry::GetFieldCount(const char *pszFieldPath, CPLErr * /* peErr */)
{
    // Is there a node path in this string?
    if (strchr(pszFieldPath, ':') != nullptr)
    {
...
GInt32 MMREntry::GetIntField(const char *pszFieldPath, CPLErr *peErr)
...
GIntBig MMREntry::GetBigIntField(const char *pszFieldPath, CPLErr *peErr)
...
double MMREntry::GetDoubleField(const char *pszFieldPath, CPLErr *peErr)
...
const char *MMREntry::GetStringField(const char *pszFieldPath, CPLErr *peErr,
...
CPLErr MMREntry::SetFieldValue(const char *pszFieldPath, char chReqType,
                               void *pValue)
...
CPLErr MMREntry::SetStringField(const char *pszFieldPath, const char *pszValue)
...
CPLErr MMREntry::SetIntField(const char *pszFieldPath, int nValue)
...
CPLErr MMREntry::SetDoubleField(const char *pszFieldPath, double dfValue)
...

{
    ...
    {
        nFilePos =
            MMRAllocateSpace(psMMR, psMMR->nEntryHeaderLength + nDataSize);

        if (nDataSize > 0)
            nDataPos = nFilePos + psMMR->nEntryHeaderLength;
    }

    // Force all children to set their position.
    for (MMREntry *poThisChild = poChild; poThisChild != nullptr;
...

{
    ...
    if (bDirty)
    {
        ...
        // Write the Emmr_Entry fields.

        // VSIFFlushL(psMMR->fp);
        if (VSIFSeekL(psMMR->fp, nFilePos, SEEK_SET) != 0)
        {
            CPLError(CE_Failure, CPLE_FileIO,
        ...
        MMRStandard(4, &nLong);
        bool bOK = VSIFWriteL(&nLong, 4, 1, psMMR->fp) > 0;
        ...
        if (poPrev != nullptr)
            ...
            nLong = 0;
        MMRStandard(4, &nLong);
        bOK &= VSIFWriteL(&nLong, 4, 1, psMMR->fp) > 0;
        ...
        bOK &= VSIFWriteL(szName, 1, 64, psMMR->fp) > 0;
        bOK &= VSIFWriteL(szType, 1, 32, psMMR->fp) > 0;
        ...
        {
            CPLError(CE_Failure, CPLE_FileIO,
                     "Failed to write MMREntry %s(%s), out of disk space?",
                     szName, szType);
            return CE_Failure;
        }
        ...
        {
            if (VSIFSeekL(psMMR->fp, nDataPos, SEEK_SET) != 0 ||
                VSIFWriteL(pabyData, nDataSize, 1, psMMR->fp) != 1)
            {
                CPLError(CE_Failure, CPLE_FileIO,
                         "Failed to write %d bytes MMREntry %s(%s) data, "
                         "out of disk space?",
...
void MMREntry::MarkDirty()

{
    bDirty = true;
    psMMR->bTreeDirty = true;
}

...
const char *MMRField::Initialize(const char *pszInput)
...
bool MMRField::CompleteDefn(MMRDictionary *poDict)
...
CPLErr MMRField::SetInstValue(const char *pszField, int nIndexValue,
...

{
    ...
    else if (chReqType == 'p')
    {
        CPLError(
            ...
            "MMRField::SetInstValue() not supported yet for pointer values.");
    ...
    switch (chItemType)
    {
        ...
        case 'b':
        {
            ...
            MMRStandard(2, &nBaseItemType);  // Swap back for our use.
            ...
            {
                if ((nIndexValue + 1) *
                        (MMRGetDataTypeBits(eBaseItemType) / 8) >
                    nDataSize)
                {
                    CPLError(CE_Failure, CPLE_AppDefined,
                ...
                else
                {
                    CPLError(CE_Failure, CPLE_AppDefined,
                             ...
                             "not currently supported.",
                             pszField, MMRGetDataTypeName(eBaseItemType));
                    return CE_Failure;
                }
            }
        }
            ...
            if (poItemObjectType != nullptr)
            {
                ...
                else
                {
                    for (int iIndexCounter = 0; iIndexCounter < nIndexValue &&
                         ...
                         iIndexCounter++)
                    {
                        std::set<MMRField *> oVisitedFields;
...
bool MMRField::ExtractInstValue(const char *pszField, int nIndexValue,
                                GByte *pabyData, GUInt32 nDataOffset,
...

{
    ...
    if (chReqType == 's')
    {
        ...
        {
            // MMREntry:: BuildEntryFromMIFObject() expects to have always 8
...
int MMRField::GetInstBytes(GByte *pabyData, int nDataSize,
                           std::set<MMRField *> &oVisitedFields)

{
    ...
    if (chItemType == 'b' && nCount != 0)  // BASEDATA
    {
        ...
        if (nColumns != 0 && nRows > INT_MAX / nColumns)
            return -1;
        if (nRows != 0 &&
            ((MMRGetDataTypeBits(eBaseItemType) + 7) / 8) > INT_MAX / nRows)
            return -1;
        if (nColumns != 0 &&
            ((MMRGetDataTypeBits(eBaseItemType) + 7) / 8) * nRows >
                INT_MAX / nColumns)
            return -1;
        if (((MMRGetDataTypeBits(eBaseItemType) + 7) / 8) * nRows * nColumns >
            INT_MAX - nInstBytes)
            return -1;

        nInstBytes +=
            ((MMRGetDataTypeBits(eBaseItemType) + 7) / 8) * nRows * nColumns;
    }
    else if (poItemObjectType == nullptr)
    {
        if (nCount != 0 &&
            MMRDictionary::GetItemSize(chItemType) > INT_MAX / nCount)
            return -1;
        if (nCount * MMRDictionary::GetItemSize(chItemType) >
            INT_MAX - nInstBytes)
            return -1;
        nInstBytes += nCount * MMRDictionary::GetItemSize(chItemType);
    }
    else
    {
...
int MMRField::GetInstCount(GByte *pabyData, int nDataSize) const
...
void MMRField::DumpInstValue(FILE *fpOut, GByte *pabyData, GUInt32 nDataOffset,
        ...
        if (bSuccess)
        {
            ...
            CPL_IGNORE_RET_VAL(VSIFPrintf(
                ...
                (nDataType >= EPT_MIN && nDataType <= EPT_MAX)
                    ? MMRGetDataTypeName(static_cast<EPTType>(nDataType))
                    : "invalid type"));
        }
        else
        {

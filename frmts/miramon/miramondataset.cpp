...
int WritePeStringIfNeeded(const OGRSpatialReference *poSRS, MMRHandle hMMR);
void ClearSR(MMRHandle hMMR);
...
void MMRRasterBand::ReadAuxMetadata()
    ...
    for (int i = 0; pszAuxMetaData[i] != nullptr; i += 4)
    {
        ...
        switch (pszAuxMetaData[i + 1][0])
        {
            case 'd':
            {
                ...
                {
                    nCount = 65536;
                    CPLDebug("MiraMonRaster", "Limiting %s to %d entries",
...
void MMRRasterBand::ReadHistogramMetadata()

{
    ...
    if (VSIFSeekL(hMMR->fp, nOffset, SEEK_SET) != 0 ||
        static_cast<int>(
            VSIFReadL(pabyWorkBuf, nBinSize, nNumBins, hMMR->fp)) != nNumBins)
    {
    ...
    for (int i = 0; i < nNumBins; i++)
        MMRStandard(nBinSize, pabyWorkBuf + i * nBinSize);
        ...
        EQUAL(poBinEntry->GetType(), "Edsc_BinFunction840"))
    {
        ...
        if (pszValue && EQUAL(pszValue, "BFUnique"))
            padfBinValues = MMRReadBFUniqueBins(poBinEntry, nNumBins);
    }
...

{
    ...
    */
    ...
    {
        MMRSetMetadata(hMMR, 0, GetMetadata());
...
CPLErr MMRDataset::WriteProjection()

{
    ...
    if (poGeogSRS)
    {
        ...
        if (!bDisablePEString)
            bPEStringStored = CPL_TO_BOOL(WritePeStringIfNeeded(&oSRS, hMMR));
    ...
    if (bForceToPEString && !bPEStringStored)
    {
        ...
        MMRSetPEString(hMMR, pszPEString);
        CPLFree(pszPEString);
    ...
    {
        MMRSetMapInfo(hMMR, &sMapInfo);
    }
    else
    {
        MMRSetGeoTransform(hMMR, sMapInfo.proName, sMapInfo.units,
                           adfGeoTransform);
    }

    if (bHaveSRS && sPro.proName != nullptr)
    {
        MMRSetProParameters(hMMR, &sPro);
        MMRSetDatum(hMMR, &sDatum);
...
void ClearSR(MMRHandle hMMR)
{
    ...
    {
        MMREntry *poMIEntry = nullptr;
        if (hMMR->papoBand[iBand]->poNode &&
            (poMIEntry = hMMR->papoBand[iBand]->poNode->GetNamedChild(
                 "Projection")) != nullptr)
        {
            poMIEntry->MarkDirty();
            ...
            MMREntry *poDatumEntry = poMIEntry->GetNamedChild("Datum");
            ...
            char *peStr = MMRGetPEString(hMMR);
...

{
    ...
    const Eprj_Datum *psDatum = MMRGetDatum(hMMR);
    const Eprj_ProParameters *psPro = MMRGetProParameters(hMMR);
    const Eprj_MapInfo *psMapInfo = MMRGetMapInfo(hMMR);

    MMREntry *poMapInformation = nullptr;
    if (psMapInfo == nullptr)
    {
        MMRBand *poBand = hMMR->papoBand[0];
              ...
              (!psPro || psPro->proZone == 0)))
    {
        ...
        char *pszPE_COORDSYS = MMRGetPEString(hMMR);
        ...
        if (pszPE_COORDSYS != nullptr && strlen(pszPE_COORDSYS) > 0 &&
            ...
            CPLTestBool(CPLGetConfigOption("MMR_USE_ESRI_PE_STRING", "YES")) &&
            oSRSFromPE.importFromWkt(pszPE_COORDSYS) == OGRERR_NONE)
        {
            ...
            if (pszProjName &&
                ...
                CPLTestBool(CPLGetConfigOption(
                    "MMR_SHOW_ESRI_PE_STRING_ONLY_WARNING", "YES")))
            {
                CPLError(CE_Warning, CPLE_AppDefined,
                         ...
                         "by setting the MMR_SHOW_ESRI_PE_STRING_ONLY_WARNING "
    ...
    auto poSRS = MMRPCSStructToOSR(psDatum, psPro, psMapInfo, poMapInformation);
    if (poSRS)
        m_oSRS = *poSRS;
    ...
    if (bTryReadingPEString)
        pszPE_COORDSYS = MMRGetPEString(hMMR);
        ...
        oSRSFromPE.importFromWkt(pszPE_COORDSYS) == OGRERR_NONE)
    {
        ...
        // Copy TOWGS84 clause from MMR SRS to PE SRS.
        if (poSRS != nullptr)
        {
            double adfCoeffs[7];
...
void MMRDataset::UseXFormStack(int nStepCount, Efga_Polynomial *pasPLForward,
    ...
    for (double dfYRatio = 0.0; dfYRatio < 1.001; dfYRatio += 0.2)
    {
        for (double dfXRatio = 0.0; dfXRatio < 1.001; dfXRatio += 0.2)
        {
            ...
            gdal::GCP gcp("", "", dfPixel, dfLine,
                          /* X = */ dfPixel,
                          /* Y = */ dfLine);
            if (MMREvaluateXFormStack(nStepCount, FALSE, pasPLReverse,
                                      &(gcp.X()), &(gcp.Y())))
            {
